  ...
  return (
    <React.Fragment>
      <form className='container-sm d-flex flex-sm-row flex-column align-items-center justify-content-center gap-4 my-3'>
        ...
        <input onChange={(e) => {setSearch(e.target.value); SetPageNumber(1); } } type='text'className="form-control" placeholder='Seach by the name' ></input>

        <button onClick={(e) => {e.preventDefault()}} className='btn btn-dark fs-6'>Search</button>
      </form>
      ...
      {/* <div>
        ...
        <input onChange={(e) => {setStatus(e.target.value); SetPageNumber(1); } } type='radio' className="" id="status" ></input>
        <input onChange={(e) => {setStatus(e.target.value); SetPageNumber(1); } } type="radio" value="Alive" name="Alive" checked={status === 'Alive' ? true : false} id="status" /> Alive
        <input onChange={(e) => {setStatus(e.target.value); SetPageNumber(1); } } type="radio" value="dead" name="dead" checked={status === 'dead' ? true : false} id="status" /> Dead
      </div> */}
